...
function practicalRuleExplanation(source: string) {
  ...
  const ruleText = normalized.replace(/^\s*(?:(?:\([a-z0-9]+\))+|[a-z0-9]+[.)-])\s*/i, "");
  ...
  if (/^Final opinions, including concurring and dissenting opinions/i.test(ruleText)) {
    return "The public can see final decisions in cases, including opinions that agree or disagree.";
  }
  if (/^Those statements of policy and interpretations/i.test(ruleText)) {
    return "The public can see agency policies and interpretations that are not published in the Federal Register.";
  }
  if (/^Administrative staff manuals and instructions to staff/i.test(ruleText)) {
